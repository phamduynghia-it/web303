...
function createNeonTexture(image, renderer) {
    ...
    let size = 256;
    if (window.devicePixelRatio > 1 && window.innerWidth > 600) {
        size = 512;
    }
    if (window.devicePixelRatio > 2 && window.innerWidth > 1000) {
        size = 1024;
    }
    const canvas = document.createElement("canvas");
    canvas.width = canvas.height = size;
...
for (let group = 0; group < numGroups; group++) {
    ...
    groupGeometryNear.setAttribute(
        ...
        new THREE.BufferAttribute(
            groupPositions.slice(0, validPointCount * 3),
            3
        )
    );
    groupGeometryNear.setAttribute(
    ...
    groupGeometryFar.setAttribute(
        "color",
        new THREE.BufferAttribute(
            groupColorsFar.slice(0, validPointCount * 3),
            3
        )
    );
...
function animatePlanetSystem() {
    if (window.textRings) {
        ...
        window.textRings.forEach((ringGroup, index) => {
            ...
            const verticalBob =
                Math.sin(
                    time * (userData.tiltSpeed * 0.7) + userData.tiltPhase
                ) * 0.3;
...
function preloadGalaxyAudio() {
    // Ưu tiên lấy thẻ audio trong HTML
...
// Thêm sự kiện phát nhạc khi chạm vào màn hình điện thoại
function enableMusicOnUserGesture() {
    if (!galaxyAudio) return;
    playGalaxyAudio();
    // Sau khi phát nhạc, bỏ lắng nghe để tránh lặp lại
    window.removeEventListener("touchstart", enableMusicOnUserGesture);
    window.removeEventListener("click", enableMusicOnUserGesture);
}
window.addEventListener("touchstart", enableMusicOnUserGesture);
window.addEventListener("click", enableMusicOnUserGesture);
